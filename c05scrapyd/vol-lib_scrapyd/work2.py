...
class QuotSpider(scrapy.Spider):
    ...
    def saveQuotesOpen(self, thedate):
        ...
        stmt = """UPDATE 
          ...
          (SELECT a.symbol, a.Date, a.Price FROM
            `tHKEX_Sales` a 
                INNER JOIN 
             (SELECT `symbol`,`Date`, MIN(`Serial`) AS minSerial FROM `tHKEX_Sales` 
              WHERE `Date` = '%s'
              GROUP BY `symbol`, `Date`) b 
              ON a.symbol = b.symbol AND a.Date = b.Date AND a.Serial = b.minSerial
          ) AS t2
        SET t1.Open = t2.Price
        WHERE t1.symbol = t2.symbol AND t1.Date = t2.Date
